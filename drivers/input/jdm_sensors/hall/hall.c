...
#define KEY_HALL_PENOUT                0x289
#define KEY_HALL_PENIN                 0x290
...
#endif
	struct workqueue_struct *hall_irq_event_wq;
	struct delayed_work hall_irq_event_work;
...
{
	int hall_gpio;
	struct hall_switch_info *hall_info=container_of(work, struct hall_switch_info, hall_irq_event_work.work);
	...
	if (hall_gpio) {
			input_report_key(hall_info->ipdev, KEY_HALL_PENOUT, 1);
			input_sync(hall_info->ipdev);
			input_report_key(hall_info->ipdev, KEY_HALL_PENOUT, 0);
			input_sync(hall_info->ipdev);
	}else{
			input_report_key(hall_info->ipdev, KEY_HALL_PENIN, 1);
			input_sync(hall_info->ipdev);
			input_report_key(hall_info->ipdev, KEY_HALL_PENIN, 0);
			input_sync(hall_info->ipdev);
	}
	enable_irq(hall_info->irq);
...
static irqreturn_t hall_interrupt(int irq, void *data)
{
	...
	queue_delayed_work(hall_info->hall_irq_event_wq, &hall_info->hall_irq_event_work,
			msecs_to_jiffies(50));
        return IRQ_HANDLED;
}
...
static int hall_power_on(struct device *pdev)//for test mawenke
{
	...
	#endif
	#if 1
	hall_vio = regulator_get(pdev, "vddio-supply");
	if (IS_ERR(hall_vio)) {
		ret = -1;
		...
		struct class_attribute *attr, char *buf)
{
	...
	if(gpio_get_value(global_hall_info->irq_gpio)){
		state = 3;
		pr_err("Hall_sensor hall_state_show far \n");
	}else{
		state = 2;
		pr_err("Hall_sensor hall_state_show near \n");
	}
...
/*input system config*/
	...
	input_set_capability(hall_info->ipdev, EV_KEY, KEY_HALL_PENOUT);
	input_set_capability(hall_info->ipdev, EV_KEY, KEY_HALL_PENIN);
	...
	INIT_DELAYED_WORK(&hall_info->hall_irq_event_work, hall_irq_event_workfunc);
	hall_info->hall_irq_event_wq = create_workqueue("hall_irq_event_wq");
...
static int hall_remove(struct platform_device *pdev)
{
	...
	destroy_workqueue(hall->hall_irq_event_wq);
	return 0;
}
