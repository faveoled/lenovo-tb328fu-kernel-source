...
static ssize_t aw9610x_onoff_sensor(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t count)
{
	ssize_t ret;
	...
	if (state == 0) {
		...
		msleep(200);
		onoff_value = 0;
		pr_info("aw9610x turn off sensor, state is:%d,irq_status_temp = 0x%x\n",state,irq_status_temp);
	}
	else {
		aw9610x_i2c_read(aw9610x, REG_SCANCTRL0, &data_en);
		...
		cal_flag=1;
		onoff_value = 1;
		pr_info("aw9610x turn on sensor, state is:%d\n",state);
	}
				...
				struct device_attribute *attr, char *buf)
{
	...
	pr_info("aw96105a cfg name is %s\n", aw9610x->cfg_name);
...
static DEVICE_ATTR(onoff_sensor, 0664, onoff_show, aw9610x_onoff_sensor);
...
static void aw9610x_irq_handle(struct aw9610x *aw9610x)
{
	...
	status = (uint8_t)((curr_status&0x3f000000) >> 24);
	...
	for (i = 0; i < 3; i++)
	{
		curr_ch[i] = (((uint8_t)(curr_status >> (24+i)) & 0x01) << 0) | //prox0
					 (((uint8_t)(curr_status >> (16+i)) & 0x01) << 1) | //prox1
					 (((uint8_t)(curr_status >> (8+i)) & 0x01) << 2) | //prox2
					 (((uint8_t)(curr_status >> (0+i)) & 0x01) << 3);  //prox3
