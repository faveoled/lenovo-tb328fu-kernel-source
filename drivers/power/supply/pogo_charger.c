...
struct pogo_charger *the_pogochg;
EXPORT_SYMBOL(the_pogochg);

static int pogo_charger_update_real_type(struct pogo_charger *pogochg);

int pogo_charger_otg_en_set(struct pogo_charger *pogochg, int enable)
{
...
EXPORT_SYMBOL(pogo_charger_otg_en_set);
...
static int pogo_charger_update_real_type(struct pogo_charger *pogochg)
{
	...
	//power_supply_set_property(pogochg->usb_psy,POWER_SUPPLY_PROP_REAL_TYPE, &val);
	power_supply_set_property(pogochg->usb_psy,POWER_SUPPLY_PROP_TYPE, &val);
...
static int pogo_charger_switcher_init(struct pogo_charger *pogochg)
{
...
#endif
	pogochg->gpio_usb_en = of_get_gpio(pogochg->dev->of_node, 2);
	pogochg->gpio_usb_int = of_get_gpio(pogochg->dev->of_node, 3);
	pogochg->gpio_pogo_en = of_get_gpio(pogochg->dev->of_node, 0);
	pogochg->gpio_pogo_int = of_get_gpio(pogochg->dev->of_node, 1);
	pogochg->gpio_pogo_ocp_int = of_get_gpio(pogochg->dev->of_node, 5);
	pogochg->gpio_dcdc_en = of_get_gpio(pogochg->dev->of_node, 4);
	...
	pr_info("%s: pogochg->gpio_pogo_ocp_int %d, pogochg->gpio_dcdc_en %d\n", __func__, pogochg->gpio_pogo_ocp_int, pogochg->gpio_dcdc_en);
	...
	gpio_direction_output(pogochg->gpio_pogo_en, 0); // enable pogo
	gpio_direction_output(pogochg->gpio_dcdc_en, 0); // dis dcdc
	gpio_direction_input(pogochg->gpio_usb_int);
	gpio_direction_input(pogochg->gpio_pogo_int);
	gpio_direction_input(pogochg->gpio_pogo_ocp_int);
