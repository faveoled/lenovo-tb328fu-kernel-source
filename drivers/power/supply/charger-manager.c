...
static void cm_update_charge_info(struct charger_manager *cm, int cmd)
{
	...
	case POWER_SUPPLY_WIRELESS_CHARGER_TYPE_EPP:
		...
		break;
	case POWER_SUPPLY_USB_CHARGER_TYPE_FLOAT:
...
static int battery_maintian_mode_adjust_cv(struct charger_manager *cm, int *term_volt)
{
	struct power_supply *psy;
	union power_supply_propval val;
	int bat_maintain_enable = 0;
	int bat_used_months = -1;
	int i;
	int bmm_cv = -1;
	struct bat_maintian_mode *bmm;

	if (cm == NULL || term_volt == NULL)
	{
		dev_err(cm->dev, "cm or term_volt null\n");
		return -1;
	}

	bmm = cm->desc->bat_maintain;

	psy = power_supply_get_by_name("sc27xx-fgu");
	if (psy)
	{
		if (power_supply_get_property(psy, POWER_SUPPLY_PROP_BAT_MAINTAIN_ENABLE, &val))
		{
			dev_err(cm->dev, "failed to get POWER_SUPPLY_PROP_BAT_MAINTAIN_ENABLE\n");
			return -1;
		}
		else
		{
			bat_maintain_enable = val.intval;
		}

		if (power_supply_get_property(psy, POWER_SUPPLY_PROP_BAT_USED_MONTHS, &val))
		{
			dev_err(cm->dev, "failed to get POWER_SUPPLY_PROP_BAT_USED_MONTHS\n");
			return -1;
		}
		else
		{
			bat_used_months = val.intval;
		}
	}
	else
	{
	...
	if (bat_maintain_enable)
	{
		for (i = 0; i < bmm->table_size; i++)
		{
			if (bat_used_months >= bmm->bat_maintian_array[i].bat_months)
			{
				break;
			}
		}

		bmm_cv = bmm->bat_maintian_array[i].bat_max_cv;
		dev_info(cm->dev, "%s: bmm_cv:%d, term_volt:%d, i:%d",
			__func__, bmm_cv, *term_volt, i);
...
static bool cm_manager_adjust_current(struct charger_manager *cm, int jeita_status)
{
	...
	battery_maintian_mode_adjust_cv(cm, &term_volt);
...
static int cm_manager_get_jeita_status(struct charger_manager *cm, int cur_temp)
{
	...
	case 2:
		...
		else if (jeita_status == STATUS_T1_TO_T2 &&
			cur_temp <= 460)
		{
	...
	case 1:
		...
		else if (jeita_status == STATUS_T0_TO_T1 &&
	...
	case 0:
		if (jeita_status == STATUS_T1_TO_T2 &&
			cur_temp >= 140)
		{

		}
		else if (jeita_status == STATUS_BELOW_T0 &&
...
static void misc_event_handler(struct charger_manager *cm, enum cm_event_types type)
{
	...
	if (cm_get_battery_temperature_by_psy(cm, &bat_temp) != 0)
		bat_temp = 0x5AA5;

	if (cm_suspended)
		device_set_wakeup_capable(cm->dev, true);

	dev_err(cm->dev, "charging_status: %d, bat_temp: %d\n",
		cm->charging_status, bat_temp);
...
int charge_tp_status = 0;
EXPORT_SYMBOL(charge_tp_status);
				...
				union power_supply_propval *val)
{
	...
	int i;
	int batt_uV;
	int bat_used_months = 0;
	int bat_maintain_en = 0;
	int fake_uisoc = 0;
	...
	case POWER_SUPPLY_PROP_STATUS:
		...
		charge_tp_status = val->intval;
		//dev_err(cm->dev,"csy---hxtp--charge--status=%d", charge_tp_status);
		break;
	...
	case POWER_SUPPLY_PROP_HEALTH:
		...
		dev_info(cm->dev, "POWER_SUPPLY_PROP_HEALTH: %d\n", val->intval);
		break;
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		if (!fuel_gauge) {
			...
			if (val->intval > 100)
				...
				val->intval = 0;
			dev_err(cm->dev, "get power_supply_get_by_name failed\n");
			break;
		}

		ret = power_supply_get_property(fuel_gauge,
						POWER_SUPPLY_PROP_BAT_MAINTAIN_ENABLE, val);
		if (ret) {
			dev_err(cm->dev, "get POWER_SUPPLY_PROP_BAT_MAINTAIN_ENABLE failed\n");
		} else {
			bat_maintain_en = val->intval;
		}

		if (bat_maintain_en) {
			ret = power_supply_get_property(fuel_gauge,
						POWER_SUPPLY_PROP_BAT_USED_MONTHS, val);
			if (ret) {
				dev_err(cm->dev, "get POWER_SUPPLY_PROP_BAT_USED_MONTHS failed\n");
			} else {
				bat_used_months = val->intval;
			}

			for (i = 0; i < cm->desc->bat_maintain->table_size; i++) {
				if (bat_used_months >= cm->desc->bat_maintain->bat_maintian_array[i].bat_months) {
					break;
				}
			}

			fake_uisoc = cm->desc->cap * 100 / cm->desc->bat_maintain->bat_maintian_array[i].full_chg_soc;
			val->intval = DIV_ROUND_CLOSEST(fake_uisoc, 10);
			if (cm->desc->bat_maintain->soc_force_set_full) {
				val->intval = 100;
			}
		} else {
			val->intval = DIV_ROUND_CLOSEST(cm->desc->cap, 10);
		...
		if (val->intval > 100)
			...
			val->intval = 0;
		dev_info(cm->dev, "bat_maintain_en:%d, bat_used_months:%d, fake_uisoc:%d, val->intval:%d\n",
			bat_maintain_en, bat_used_months, fake_uisoc, val->intval);
		break;
	...
	case POWER_SUPPLY_PROP_TIME_TO_FULL_NOW:
		...
		if (bat_maintain_en) {
			...
			if (cm->desc->cap >= (cm->desc->bat_maintain->bat_maintian_array[i].full_chg_soc * 10)) {
				val->intval = 0;
			} else {
				val->intval = ((cm->desc->bat_maintain->bat_maintian_array[i].full_chg_soc
					* 10 - cm->desc->cap) * total_cap *
					cm->desc->bat_maintain->bat_maintian_array[i].full_chg_soc
					/ 100 / 1000) * 3600 / chg_cur;
			}
		} else {
			val->intval =
			((1000 - cm->desc->cap) * total_cap / 1000) * 3600 / chg_cur;
		     ...
		     const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT:
		for (i = 0; cm->desc->psy_charger_stat[i]; i++) {
			...
			dev_err(cm->dev, "%s:psy:%s, val:%d\n", __func__, psy->desc->name, val->intval);
	...
	case POWER_SUPPLY_PROP_CHARGE_CONTROL_LIMIT:
		...
		for (i = 0; cm->desc->psy_charger_stat[i]; i++) {
			...
			dev_err(cm->dev, "%s:psy:%s, thermal_val:%d\n", __func__, psy->desc->name, thermal_val.intval);
...
static int cm_init_battery_maintain_mode_table(struct charger_desc *desc, struct device *dev)
{
	struct device_node *np = dev->of_node;
	struct bat_maintian_mode *bmm;
	const __be32 *list;
	int i, size;
	struct bat_maintian_table *table;

	list = of_get_property(np, "bat-maintian-table", &size);
	if (!list || !size) {
		dev_err(dev, "no list or no size\n", __func__);
		return 0;
	}

	dev_info(dev, "%s size:%d\n", __func__, size);
	size = size / (sizeof(struct bat_maintian_table));

	bmm = devm_kzalloc(dev, sizeof(struct bat_maintian_table) * size
		+ sizeof(int) + sizeof(int), GFP_KERNEL);
	if (!bmm) {
		dev_err(dev, "%s bmm alloc fail\n", __func__);
		return -ENOMEM;
	}

	table = devm_kzalloc(dev, sizeof(struct bat_maintian_table) * size, GFP_KERNEL);
	if (!table) {
		dev_err(dev, "%s table alloc fail\n", __func__);
		return -ENOMEM;
	}

	bmm->table_size = size;
	bmm->soc_force_set_full = 0;

	for (i = 0; i < bmm->table_size; i++) {
		table[i].bat_months = be32_to_cpu(*list++);
		table[i].bat_max_cv = be32_to_cpu(*list++);
		table[i].full_chg_soc = be32_to_cpu(*list++);
	}

	bmm->bat_maintian_array = table;
	desc->bat_maintain = bmm;
...
static struct charger_desc *of_cm_parse_desc(struct device *dev)
{
	...
	ret = cm_init_battery_maintain_mode_table(desc, dev);
	if (ret)
		return ERR_PTR(ret);
...
extern int bq2560x_dump_register_myself(void);
...
static void charger_manager_shutdown(struct platform_device *pdev)
{
	...
	if (cm->desc->uvlo_trigger_cnt < CM_UVLO_CALIBRATION_CNT_THRESHOLD) {
		set_batt_cap(cm, cm->desc->cap);
		dev_info(&pdev->dev, "%s cm->desc->cap:%d\n", __func__, cm->desc->cap);
	}
}
